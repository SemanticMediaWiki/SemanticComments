...
/*
var bind = function(context, method) {return function() {return method.apply(context, arguments);}; };
*/
...
function CECommentForm() {
	...
	this.processFormCallback = function( data, textStatus, jqXHR) {
		...
		var htmlmsg = htmlmsgs && htmlmsgs[0] ? htmlmsgs[0].firstChild.nodeValue : null;
...
function CPendingIndicator( container ) {
	this.constructor = function( container ) {
		...
		$( this.pendingIndicator ).attr(
			...
			'/extensions/SemanticComments/skins/Comment/icons/ajax-loader.gif'
		);
 ...
 * Browser tools
 */
function SemanticCommentsBrowserDetectLite() {
 ...
 * XML Tools
 */
SemanticCommentsXMLTools = new Object();

/**
 * Creates an XML document from string
 */
SemanticCommentsXMLTools.createDocumentFromString = function( xmlText ) {
